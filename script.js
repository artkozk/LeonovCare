(() => {
  ...
  const groupLink = config.groupUrl || "#";
  const botContactLink = config.botUrl || "#";
  ...
  const appendFooterContacts = () => {
    const footerInner = document.querySelector(".site-footer .footer-inner");
    if (!footerInner || footerInner.querySelector("[data-footer-contacts]")) {
      return;
    }
    ...
    const links = [
      ...
      { label: "YouTube", href: youtubeLink, text: "@leonov_care" }
    ...
    list.className = "footer-links footer-contacts";
    list.setAttribute("data-footer-contacts", "true");
    list.innerHTML = links.map(
      (item) => `<a href="${item.href}" target="_blank" rel="noopener noreferrer">${item.label}: ${item.text}</a>`
    ).join("");
    footerInner.appendChild(list);
  };
