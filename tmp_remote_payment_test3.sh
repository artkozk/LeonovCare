...
client = CardlinkClient(
    ...
    return_url=env.get('CARDLINK_RETURN_URL',''),
    timeout_sec=30,
    api_base_url=env.get('CARDLINK_API_BASE_URL') or 'https://cardlink.link/api',
)
try:
    ...
    print('cardlink_ok', p.get('id'), p.get('confirmation_url'))
except Exception as e:
